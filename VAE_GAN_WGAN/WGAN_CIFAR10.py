...
# cross_entropy = tf.keras.losses.BinaryCrossentropy(from_logits=Ture)

lam = 10.

def discriminator_loss(real_output, fake_output,images,generated_images):
    ...
    # interpolated = images + alpha*(generated_images-images)
    # inte_logit = discriminator(interpolated)
    # gradients = tf.gradients(inte_logit,[interpolated,])[0]
    # grad_l2 = tf.sqrt(tf.reduce_sum(tf.square(gradients),axis=[1,2,3]))
    # gradient_penalty = tf.reduce_mean((grad_l2-1)**2)
    # total_loss += lam*gradient_penalty
...
def train_step(images):
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
      ...
      disc_loss = discriminator_loss(real_output, fake_output,images,generated_images)
...
def train(dataset, epochs):
  iteration = 0
  batch_per_epoch = int(train_images.shape[0] / BATCH_SIZE)
  for epoch in range(epochs):
    dataset_pro = tf.data.Dataset.from_tensor_slices(dataset).shuffle(BUFFER_SIZE).batch(BATCH_SIZE)
    start = time.time()
    index = 0
    for image_batch in dataset_pro:
      if(index==batch_per_epoch-1):
        continue
...
train_log_dir = 'logs/' + current_time + '/GAN_MNIST_l' + str(latent_size) + '_h' + str(hidden_layer_num)
